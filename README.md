...
模型名称的日级报表按北京时间 **2026-09-10** 切换归类；分时明细则在同步时观测到精确的 `deepseek-flash`，且其后不再出现旧 V4 ID 后，从观测到的首个小时开始切换。切换前的旧 ID 保留为 **V4 Flash** / **V4 Flash Vision**，切换后旧兼容别名与 V4.1 Flash 统一显示为 **V4.1 Flash**。V4.1 Flash 本身支持图片识别，不另行拆分视觉模型；未知模型 ID 会原样保留。**V4 Pro** 始终独立统计。合并只作用于图表与报表，数据库继续保留平台返回的原始模型 ID。
