...
// 桌宠图形简单（PNG + SVG 动画），禁用硬件加速可大幅降低 GPU 进程内存占用
...
function createPetWindow() {
  ...
  petWin.once('ready-to-show', () => {
    if (!petWin) return
    petWin.showInactive()
    rebuildTrayMenu() // 首次显示后重建托盘菜单，使「显示鲸鱼」勾选状态正确
  })
  petWin.on('closed', () => {
    petWin = null
    app.quit()
...
// 打开用量统计窗口（按需创建）。
function openUsage() {
  if (!usageWin || usageWin.isDestroyed()) createUsageWindow()
...
// ================================ 平台令牌登录窗 ===========================
// 打开 platform.deepseek.com，用户登录后从 localStorage 提取 userToken。
function extractTokenJS() {
  return `(function () {
    function unwrap(v) {
      if (!v) return null;
      v = String(v).trim();
    ...
    try {
      ...
      if (/sign|login|auth|verify|captcha|error/i.test(path)) return '';
...
// 打开平台登录窗，登录成功后返回提取到的 userToken（校验通过才关闭窗口）。
function openPlatformLogin() {
  return new Promise((resolve) => {
    ...
    // 本地尚未配置令牌时：首次加载清掉平台残留会话，避免误抓旧 token 导致窗口秒关
    let cleared = !!(configMod.getEffective() && configMod.getEffective().platformToken)
    const doClear = async () => {
      if (cleared) return
      cleared = true
      try {
        await win.webContents.executeJavaScript('try{localStorage.clear();sessionStorage.clear();}catch(e){}', true)
      } catch (e) {}
      try { await win.webContents.session.clearStorageData({ storages: ['cookies', 'localstorage'] }) } catch (e) {}
    ...
    poll = setInterval(async () => {
      ...
      let token = ''
      try {
        token = await win.webContents.executeJavaScript(extractTokenJS(), true)
      } catch (e) { /* 页面未就绪 */ }
      if (!token || typeof token !== 'string' || !token.trim()) {
        if (!cleared) await doClear()
        return
      }
      validating = true
      const v = await usageMod.validateToken(token.trim())
      validating = false
      if (v && v.ok) finish(token.trim())
      // 校验失败（旧 token / 登录未完成）→ 继续轮询
    }, 2000)
    timeout = setTimeout(() => finish(null), 5 * 60 * 1000)
  })
}
...
// 清洗渲染进程传来的形状矩形，统一为 setShape 需要的 {x,y,width,height} 整数格式。
// 注意：之前误用 {w,h} 浮点，导致 setShape 无效、全窗口拦截鼠标（透明区「失效」）。
function sanitizeRects(rects) {
  ...
  for (const r of rects) {
    ...
    const w = Math.round(Number(r.width !== undefined ? r.width : r.w))
    const h = Math.round(Number(r.height !== undefined ? r.height : r.h))
...
function registerIpc() {
  ...
  ipcMain.handle('usage:summary', () => {
    let wallets = null
    const raw = storeMod.getMeta('balance_wallets')
    if (raw) { try { wallets = JSON.parse(raw) } catch (e) { wallets = null } }
    return {
      summary: storeMod.getSummary(),
      range: storeMod.getDateRange(),
      balance: wallets,
      lastSyncAt: storeMod.getMeta('last_sync_at') || null,
    }
  })
