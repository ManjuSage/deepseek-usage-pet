...
(function () {
  ...
  function hideBubble() {
    ...
    if (bubbleSwapTimer) { clearTimeout(bubbleSwapTimer); bubbleSwapTimer = null }
    if (hintFadeTimer) { clearTimeout(hintFadeTimer); hintFadeTimer = null }
    if (shapeTimer) { clearTimeout(shapeTimer); shapeTimer = null }
    textBox.style.transition = ''
    textBox.style.opacity = ''
    hintEl.style.transition = ''
    hintEl.style.opacity = ''
  ...
  bubbleBox.addEventListener('click', function (e) {
    ...
    // 大多数（80%）：显示峰谷状态；小概率（20%）：随机台词
    bubbleRandomActive = true
    bubbleRandomLines = Math.random() < 0.2 ? pickRandomLines() : buildGroup1()
  ...
  function render() {
    ...
    if (state.status === 'error') {
      ...
      hint = state.message ? state.message : '获取失败 · 点击重试'
  ...
  function reportShape() {
    try {
      ...
      if (w > 0) {
        if (whaleBounds) {
          // 用鲸鱼 alpha 轮廓包围盒（去掉透明顶部/侧边，更贴合身体）
          var s = w / 610
          var bx = img.offsetLeft + whaleBounds.x0 * s
          var by = img.offsetTop + whaleBounds.y0 * s
          var bw = (whaleBounds.x1 - whaleBounds.x0 + 1) * s
          var bh = (whaleBounds.y1 - whaleBounds.y0 + 1) * s
          p(bx - pad, by - pad, bw + pad * 2, bh + pad * 2)
        } else {
      ...
      if (bubbleShown) {
        ...
        if (b > 0) {
          // 气泡贴合椭圆轮廓（去掉四角与右侧透明区）
          var bh0 = bubbleBox.offsetHeight
          var bx0 = bubbleBox.offsetLeft + b * 0.06
          var by0 = bubbleBox.offsetTop + bh0 * 0.02
          var bx1 = bubbleBox.offsetLeft + b * 0.82
          var by1 = bubbleBox.offsetTop + bh0 * 0.96
          p(bx0 - pad, by0 - pad, (bx1 - bx0) + pad * 2, (by1 - by0) + pad * 2)
        }
      }
  ...
  var hitCanvas = null
  var hitReady = false
  var whaleBounds = null // 鲸鱼 alpha 轮廓包围盒 {x0,y0,x1,y1}（610 空间）
  function computeWhaleBounds() {
    if (!hitCanvas || !hitReady) return null
    try {
      var data = hitCanvas.getContext('2d').getImageData(0, 0, 610, 610).data
      var x0 = 610, y0 = 610, x1 = -1, y1 = -1
      for (var y = 0; y < 610; y++) {
        for (var x = 0; x < 610; x++) {
    ...
    try {
      ...
      probe.onload = function () {
        try {
          ...
          hitReady = true
          whaleBounds = computeWhaleBounds()
          reportShape() // 用更贴合轮廓的包围盒更新命中区
        } catch (err) {}
      }
  ...
  // 窗口被 setShape 裁剪后，鼠标移出鲸鱼区域（进入透明区/离开窗口）时
  // 不再收到 pointermove，这里用 mouseleave 兜底隐藏菜单按钮并复位光标。
  ...
  async function applyConfig(c, first) {
    ...
    var bi = (typeof c.bubbleInterval === 'number' && isFinite(c.bubbleInterval)) ? Math.max(0, Math.round(c.bubbleInterval)) : 900
    bi = bi * 1000
