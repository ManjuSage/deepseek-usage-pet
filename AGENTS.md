...
这是一个**独立改造版**的 DeepSeek 桌面鲸鱼桌宠：在透明置顶鲸鱼（余额气泡 + 拖拽 + Q 弹）基础上，扩展了**托盘**、**平台令牌登录**、**用量统计面板**（历史回填 + 日级/分时图表 + 余额 + 同步时间）。已与原项目 [momo-OwO-qwq/DeepSeek-Whale-Pet](https://github.com/momo-OwO-qwq/DeepSeek-Whale-Pet)（MIT）分离，是另一个项目；用量统计参照 [33March7/deepseek-api-usage-statistics](https://github.com/33March7/deepseek-api-usage-statistics)（Unlicense）。
