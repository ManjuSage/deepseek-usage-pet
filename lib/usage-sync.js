...
const FLASH_CUTOVER_META = 'flash_cutover_observed_at'
const FLASH_NEW_MODEL = 'deepseek-flash'
const FLASH_LEGACY_MODELS = new Set(['deepseek-v4-flash', 'deepseek-v4-flash-vision-exp'])
...
function compareHourBucket(a, b) {
  if (a.date !== b.date) return a.date < b.date ? -1 : 1
  return a.hour - b.hour
}

// 只有精确观测到 deepseek-flash，且该小时及之后不再出现旧 V4 ID，才确认切换。
function detectFlashCutover(rows) {
  const buckets = new Map()
  for (const row of rows || []) {
    if (!row || !row.utc_date || !Number.isInteger(Number(row.hour))) continue
    const bucket = { date: String(row.utc_date), hour: Number(row.hour) }
    const key = bucket.date + '/' + bucket.hour
    if (!buckets.has(key)) buckets.set(key, { ...bucket, models: new Set() })
    buckets.get(key).models.add(row.model)
  }
  const ordered = [...buckets.values()].sort(compareHourBucket)
  for (const candidate of ordered) {
    if (!candidate.models.has(FLASH_NEW_MODEL)) continue
    const legacyAfter = ordered.some((bucket) =>
      compareHourBucket(bucket, candidate) >= 0 && [...bucket.models].some((model) => FLASH_LEGACY_MODELS.has(model))
    )
    if (!legacyAfter) return { date: candidate.date, hour: candidate.hour }
  }
  return null
}

function observeFlashCutover(rows) {
  const observed = detectFlashCutover(rows)
  if (!observed || store.getMeta(FLASH_CUTOVER_META)) return observed
  store.setMeta(FLASH_CUTOVER_META, observed)
  log.info('[usage] 观测到 Flash 路由切换: ' + observed.date + ' ' + String(observed.hour).padStart(2, '0') + ':00')
  return observed
}
...
async function syncHourly(token) {
  ...
  for (const d of [gmt8Today(), gmt8Yesterday()]) {
    ...
    observedRows.push(...costRows)
    nC += costRows.length
  }
  observeFlashCutover(observedRows)
...
// 请求取消正在进行的用量同步（主要在历史回填阶段生效）。
function cancelSync() {
  cancelRequested = true
}

async function syncBackfill(token, onProgress) {
  let zeroStreak = 0
  let amountTotal = 0
  let costTotal = 0
  ...
  let first = true
  for (let i = 0; i < 120; i++) {
    if (cancelRequested) return { amount: amountTotal, cost: costTotal, cancelled: true }
    ...
    if (onProgress) onProgress({ segment: i + 1, startDate })
    log.debug('[usage] 回填段 ' + (i + 1) + ' ' + startDate)
...
async function syncAll(token, onProgress) {
  ...
  store.beginBatch()
  try {
    const backfill = await syncBackfill(token, onProgress)
    if (backfill.cancelled) {
      // 用户取消：跳过后续分时/余额同步，尽早落盘已回填的部分
      store.setMeta('last_sync_at', new Date().toISOString())
      store.flush()
      return { backfill, cancelled: true }
    }
    const hourly = await syncHourly(token)
    const balance = await syncBalance(token)
    store.setMeta('last_sync_at', new Date().toISOString())
    store.flush()
    return { backfill, hourly, balance, cancelled: false }
  } catch (err) {
    store.flush()
    throw err
  }
