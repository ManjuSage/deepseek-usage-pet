...
const PRICING = {
  'deepseek-chat & deepseek-reasoner': BASE_PRICE, // 平台把旧版 chat/reasoner 合并成一个名字返回
...
function priceFor(model) {
  ...
  // 带版本/日期后缀的临时模型（如 deepseek-v4.1-flash-expires-on-0910）按「家族」识别，
  // 避免每个新模型都要手动登记、并避免刷新时反复告警。
  if (m.indexOf('-pro') !== -1) return PRO_PRICE
  if (m.indexOf('-flash') !== -1 || m.indexOf('-chat') !== -1 || m.indexOf('-reasoner') !== -1 || m.indexOf('-vision') !== -1) return BASE_PRICE
  // 真正未知的模型：同一名字只告警一次，防止刷屏
  if (!warnedModels.has(m)) {
    warnedModels.add(m)
    log.warn('[price] unknown model, using default price: ' + m)
  }
...
async function fetchUsage(platformToken) {
  try {
    ...
    const url = USAGE_URL_BASE + '?start=' + start + '&end=' + end + '&tz=' + TZ_OFFSET_SEC
...
class BalanceService {
  ...
  async compute(cfg) {
    ...
    // 记账模式：配置了平台令牌时，每次刷新都与平台今日用量对账，取两者较大值，
