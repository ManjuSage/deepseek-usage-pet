...
function writePool(data) {
  const sanitized = sanitizePool(data)
  try {
    fs.mkdirSync(config.CONFIG_DIR, { recursive: true, mode: 0o700 })
    const tmp = LINES_FILE + '.tmp'
    fs.writeFileSync(tmp, JSON.stringify(sanitized, null, 2), 'utf8')
    fs.renameSync(tmp, LINES_FILE)
  } catch (err) {
    // 写失败仍返回内存结果（本次生效，重启后回退）
  }
  return sanitized
}

module.exports = { LINES_FILE, DEFAULT_POOL, sanitizePool, readPool, writePool }
