...
// 平台将 deepseek-v4-flash 改名为 deepseek-flash；保留原始数据，仅在统计查询时合并。
const MODEL_GROUP_SQL = "CASE WHEN model IN ('deepseek-v4-flash', 'deepseek-flash') THEN 'deepseek-v4-flash' ELSE model END"
...
function getDailyByModel(start, end) {
  return withCacheHitRate(queryAll(
    `SELECT utc_date, ${MODEL_GROUP_SQL} AS model,
     ...
     GROUP BY utc_date, ${MODEL_GROUP_SQL} ORDER BY utc_date, model`,
    [start, end]
  ))
}
...
function getModelTotals(start, end) {
  const tokens = queryAll(
    `SELECT ${MODEL_GROUP_SQL} AS model,
     ...
     FROM amount_daily WHERE utc_date BETWEEN ? AND ? AND type != 'request_count'
     GROUP BY ${MODEL_GROUP_SQL} ORDER BY model`,
    [start, end]
  )
  const costs = queryAll(
    `SELECT ${MODEL_GROUP_SQL} AS model, currency, SUM(cost) AS cost
     FROM cost_daily WHERE utc_date BETWEEN ? AND ?
     GROUP BY ${MODEL_GROUP_SQL}, currency ORDER BY model`,
    [start, end]
  )
  const map = new Map()
...
function getDailyCostByModel(start, end) {
  return queryAll(
    `SELECT utc_date, ${MODEL_GROUP_SQL} AS model, SUM(cost) AS cost
     FROM cost_daily WHERE utc_date BETWEEN ? AND ?
     GROUP BY utc_date, ${MODEL_GROUP_SQL} ORDER BY utc_date, model`,
    [start, end]
  )
...
function getHourlyDetail(day, dim) {
  ...
  if (dim !== 'type') {
    const col = dim === 'model' ? MODEL_GROUP_SQL : 'api_key_name'
