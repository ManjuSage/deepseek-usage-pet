...
| `/api/v0/usage/by_api_key/cost?start=&end=&tz=` | 费用金额（用量面板 + 桌宠今日已用） |
...
4. **用量统计**：历史回填、近 7/30/90/365 天或自定义日期、输入缓存命中率总览、每日用量走势（按模型/按计费类型/按 API Key，支持 Tokens/费用/缓存命中率切换）+ 各模型占比饼图 + 累计趋势 + 用量热力图 + 分时下钻（弹窗）、充值余额卡片、上次同步时间（GMT+8）。
...
21. **模型路由切换**：日级接口只有日期桶，按北京时间 `2026-09-10` 起将旧 Flash 兼容别名与 `deepseek-flash`、V4.1 Flash 统一归一为 V4.1 Flash；分时接口不硬编码发布时间，而是在同步时精确观测到 `deepseek-flash` 且其后不再出现旧 V4 ID 后，持久化首个切换小时，并保持该切换点单调不回退。切换前的旧 ID 保留为旧 V4 Flash / V4 Flash Vision，V4 Pro 始终独立。V4.1 Flash 自身支持图片识别，不单独拆分视觉模型；归一化不改写 SQLite 原始模型 ID，未知模型 ID 原样保留。
